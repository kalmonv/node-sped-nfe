...
var _Tools_instances,
  _Tools_cert,
  _Tools_pem,
  _Tools_config,
  _Tools_getSignature,
  _Tools_gerarQRCodeNFCe,
  _Tools_descEvento,
  _Tools_xmlValido,
  _Tools_certTools,
  _Tools_limparSoap;
...
class Tools {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        let tempUF = urlEventos(
          __classPrivateFieldGet(this, _Tools_config, "f").UF,
          __classPrivateFieldGet(this, _Tools_config, "f").versao
        );
        const req = https.request(
          tempUF[`mod${__classPrivateFieldGet(this, _Tools_config, "f").mod}`][
            __classPrivateFieldGet(this, _Tools_config, "f").tpAmb == 1
              ? "producao"
              : "homologacao"
          ].NFeAutorizacao,
          {
        ...
        req.setTimeout(
          __classPrivateFieldGet(this, _Tools_config, "f").timeout * 1000,
          () => {
            reject({
              name: "TimeoutError",
    ...
    return new Promise(async (resvol, reject) => {
      ...
      if (data.tag == "infNFe") {
        if (xml.NFe.infNFe.ide.mod * 1 == 65) {
          xml.NFe.infNFeSupl.qrCode = __classPrivateFieldGet(
            this,
            _Tools_instances,
            "m",
            _Tools_gerarQRCodeNFCe
            ...
            xml.NFe,
            "2",
            __classPrivateFieldGet(this, _Tools_config, "f").CSCid,
            __classPrivateFieldGet(this, _Tools_config, "f").CSC
          );
  ...
  async getCertificado() {
    return new Promise(async (resvol, reject) => {
      __classPrivateFieldGet(this, _Tools_instances, "m", _Tools_certTools)
        .call(this)
        .then(resvol)
        .catch(reject);
    });
  ...
  consultarNFe(chNFe) {
    return new Promise(async (resolve, reject) => {
      ...
      if (["AC", "ES", "RN", "PB", "SC"].includes(UF)) UF = "SVRS";
      if (
        typeof __classPrivateFieldGet(this, _Tools_config, "f").tpAmb ===
        "undefined"
      )
  ...
  async sefazEvento({
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (
          !__classPrivateFieldGet(this, _Tools_config, "f").CNPJ &&
          !__classPrivateFieldGet(this, _Tools_config, "f").CPF
        )
        ...
        let detEvento = {
          "@versao": "1.00",
          descEvento: __classPrivateFieldGet(
            this,
            _Tools_instances,
            "m",
            _Tools_descEvento
        ...
        const cOrgao = !["210200", "210210", "210220", "210240"].includes(
          tpEvento
        )
          ? chNFe.substring(0, 2)
          : "91";
        ...
        await __classPrivateFieldGet(
          ...
          .call(this, xmlSing, `envEvento_v1.00`)
          .catch(reject); //Validar corpo
        ...
        try {
          const req = https.request(
            tempUF[`mod${chNFe.substring(20, 22)}`][
              __classPrivateFieldGet(this, _Tools_config, "f").tpAmb == 1
                ? "producao"
                : "homologacao"
            ].NFeRecepcaoEvento,
      ...
      try {
        ...
        var xmlSing = await json2xml({
          distDFeInt: {
            ...
            cUFAutor:
              UF2cUF[__classPrivateFieldGet(this, _Tools_config, "f").UF], // "AN" - Ambiente Nacional
            CNPJ: __classPrivateFieldGet(this, _Tools_config, "f").CNPJ,
        ...
        await __classPrivateFieldGet(
          ...
          .call(this, xmlSing, `distDFeInt_v1.01`)
          .catch(reject); //Validar corpo
        const tempUF = urlEventos(
          `AN`,
          __classPrivateFieldGet(this, _Tools_config, "f").versao
  ...
  async sefazStatus() {
    return new Promise(async (resolve, reject) => {
      if (
        typeof __classPrivateFieldGet(this, _Tools_config, "f").UF ==
        "undefined"
      )
      ...
      if (
        typeof __classPrivateFieldGet(this, _Tools_config, "f").mod ==
        "undefined"
      )
  ...
  async validarNFe(xml) {
    return new Promise((resolve, reject) => {
      __classPrivateFieldGet(this, _Tools_instances, "m", _Tools_xmlValido)
        .call(
          this,
          xml,
          `nfe_v${__classPrivateFieldGet(this, _Tools_config, "f").versao}`
        )
        .then(resolve)
        .catch(reject);
    });
...
(_Tools_cert = new WeakMap()),
  (_Tools_pem = new WeakMap()),
  (_Tools_config = new WeakMap()),
  (_Tools_instances = new WeakSet()),
  (_Tools_getSignature =
    //Responsavel por gerar assinatura
    async function _Tools_getSignature(xmlJSON, tag) {
      return new Promise(async (resvol, reject) => {
        let tempPem = await __classPrivateFieldGet(
          this,
          _Tools_instances,
          "m",
          _Tools_certTools
  ...
  (_Tools_gerarQRCodeNFCe = function _Tools_gerarQRCodeNFCe(
    NFe,
    versaoQRCode = "2",
    idCSC,
    CSC
    ...
    if (NFe.infNFe.ide.tpEmis == 1) {
      ...
      let hexDigestValue = Buffer.from(
    ...
    async function _Tools_xmlValido(xml, xsd) {
      return new Promise((resolve, reject) => {
        ...
        const schemaPath = path.resolve(
          __dirname,
          `../../schemas/PL_010_V1/${xsd}.xsd`
        );
        const verif = spawnSync(
          __classPrivateFieldGet(this, _Tools_config, "f").xmllint,
        ...
        if (verif.error) {
          ...
          return reject(
            verif.stderr
              .replace(/\/tmp\/[^:\s]+\.xml/g, "") // Remove os caminhos /tmp/*.xml
  ...
  (_Tools_certTools = function _Tools_certTools() {
    return new Promise(async (resvol, reject) => {
      ...
      pem.readPkcs12(
        __classPrivateFieldGet(this, _Tools_cert, "f").pfx,
        { p12Password: __classPrivateFieldGet(this, _Tools_cert, "f").senha },
        (err, myPem) => {
